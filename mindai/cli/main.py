"""
MindAI CLI - Command-line interface for interacting with MindAI
"""
...
def chat(query, provider, model, api_key):
    ...
    async def run():
        ...
        result = await mindai.process(query_text)
...
def memory_search(query, limit):
    ...
    async def run():
        ...
        results = await mindai.search_memory(query, limit=limit)
...
def memory_add(content, type, importance):
    """Add a memory"""
    async def run():
        from mindai.core.memory import NodeType
        
        mindai = MindAI()
        await mindai.initialize()
        
        node_type = NodeType(type)
        node_id = await mindai.add_memory(
            content=content,
            node_type=node_type,
